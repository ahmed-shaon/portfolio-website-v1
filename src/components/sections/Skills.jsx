...
const Skills = () => {
    ...
    const getLevelColor = (level) => {
        const colors = {
            ...
            "Intermediate": "text-emerald-400 bg-emerald-500/20 bordr-emerald-500/30"
        }
  ...
  return (
    <section id='skills' className='relative py-20 bg-black overflow-hidden'>
        ...
        <div className='absolute inset-0 overflow-hidden'>
            <div className='absolute top-1/4 left-0 w-96 bg-primary/10 rounded-full blur-3xl opactiy-50'></div>
                        ...
                        <FadeIn key={category} delay={categoryIndex*100}>
                            <div className='relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300 group'>
                                <div className='flex items-center gap-3 mb-6 pb-4 border-b border-white/10'>
                                    ...
                                    <h3 className='text-cl font-medium text-white'>{category}</h3>
                                </div>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className='flex items-center justify-between'>
                                                        ...
                                                        <span className={`text-xs px-2 rounded-full border ${getLevelColor(skill.level)}`}>
                                                            {skill.level}
                                                        </span>
                                                    </div>
                                ...
                                {/* Hover Glow Effect */}
                                <div className='absolute inset-0 bg-linear-to-br from-primary/0 to-primary/5 group-hover:from-primary/5 group-hover:to-primary/5 rounded-2xl transition-all duratoin-300 pointer-events-none'></div>
                            </div>
                        </FadeIn>
                    ))
                }
